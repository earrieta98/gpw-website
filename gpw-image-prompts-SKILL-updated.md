...
After the user picks a tool, generate these deliverables in this exact order:

1. **Spanish summary (Resumen)** — A plain-Spanish description of what the image will show (3-5 lines max). This is NOT a translation of the prompt — it's a quick readable summary so the user can scan it, understand the scene, and request changes before copying the English prompt. Write it conversationally, like explaining to a colleague: "Un técnico norteño insertando un servidor 2U en un rack 42U, toma media con fondo de estaciones de trabajo, iluminación LED industrial..."
2. **English prompt (Prompt)** — The actual prompt optimized for the chosen tool's syntax and strengths. This is what the user copies and pastes into the generator. Always in English, inside a code block for easy copying.
3. **Tool-specific parameters** — exact settings to use
4. **Negative prompt** (if the tool supports it)
5. **Post-production notes** — what to fix or adjust after generation
...
### Language Rule
**Every prompt delivery has TWO parts:** a Spanish summary for quick review, and an English prompt for copy-paste into the generator. The user communicates in Spanish and thinks in Spanish — the Spanish summary lets them scan the scene description and catch issues ("eso no es lo que quiero") before committing to generating images. The English prompt is what actually goes into the AI tool. AI image generators produce significantly better results with English prompts because their training data is predominantly English. Never write the actual generator prompt in Spanish — only the summary.
...
- **No wristwatches, bracelets, or jewelry on wrists** — prohibited on the assembly line for safety and ESD compliance. FLUX adds watches/bracelets by default from training data even when not prompted. Since FLUX doesn't support negative prompts, the fix is to **positively describe bare wrists**: always include "bare wrists with only a blue ESD wrist strap on his/her left wrist" in the PPE section of every prompt that shows a worker's hands/arms. This "occupies" the wrist in FLUX's attention so it doesn't fill it with a watch.
- **ESD smock is OPTIONAL** — the default for GPW images is the **polo shirt only** (no smock/bata). Only add the ESD smock when the user explicitly requests it or the scene involves handling highly ESD-sensitive components like bare PCBs or semiconductor wafers
- The base uniform rules (teal color, safety glasses, wrist strap) still apply
...
### 2026-03-28 — Box Build Integration Card (800×600)
- **Tried:** Prompt with "no wristwatch" and standard ESD wrist strap description → FLUX still rendered watches and bracelets on both wrists
- **Root cause:** FLUX has no negative prompt support and its training data associates "technician" with wristwatch. Saying "no watch" wastes words and doesn't work.
- **Fix:** Positively describe bare wrists: "bare wrists with only a blue ESD wrist strap on his left wrist" — this fills FLUX's attention for the wrist area with the ESD strap instead of letting it default to a watch. Added as permanent rule in Uniform Corrections section.