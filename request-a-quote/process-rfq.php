...
// ── Secret (loaded out of source — NEVER hardcode) ──────
// The reCAPTCHA SECRET key must not live in version control. It is read
// from the GPW_RECAPTCHA_SECRET environment variable (preferred — set it
// in cPanel), or from an untracked rfq-secrets.php (see
// rfq-secrets.example.php). The public site key stays in the HTML; only
// the secret is sensitive.
$rfq_secret = getenv('GPW_RECAPTCHA_SECRET') ?: '';
if ($rfq_secret === '' && is_file(__DIR__ . '/rfq-secrets.php')) {
    $secrets = require __DIR__ . '/rfq-secrets.php';
    $rfq_secret = is_array($secrets) ? ($secrets['recaptcha_secret'] ?? '') : '';
}

// ── Config ──────────────────────────────────────────────
$config = [
    'recaptcha_secret'   => $rfq_secret,
...
if ($config['recaptcha_secret'] === '') {
    error_log('RFQ: missing reCAPTCHA secret (set GPW_RECAPTCHA_SECRET or create rfq-secrets.php).');
    rfq_fail(500, 'The form is temporarily unavailable. Please email sales@gpw-solutions.com directly.');
}
